...
type GetBlockTemplateRequestMessage struct {
	...
	// Which kestrelcoin address should the coinbase block reward transaction pay into
...
type GetBlockTemplateResponseMessage struct {
	...
	// Whether kestrelcoind thinks that it's synced.
	// Callers are discouraged (but not forbidden) from solving blocks when kestrelcoind is not synced.
	// That is because when kestrelcoind isn't in sync with the rest of the network there's a high
...
type GetConnectedPeerInfoMessage struct {
	...
	// Whether this kestrelcoind initiated the connection
	...
	// The timestamp of when this peer connected to this kestrelcoind
...
type GetCoinSupplyResponseMessage struct {
	...
	MaxSium         uint64    `protobuf:"varint,1,opt,name=maxSium,proto3" json:"maxSium,omitempty"` // note: this is a hard coded maxSupply, actual maxSupply is expected to deviate by upto -5%, but cannot be measured exactly.
	CirculatingSium uint64    `protobuf:"varint,2,opt,name=circulatingSium,proto3" json:"circulatingSium,omitempty"`
	Error            *RPCError `protobuf:"bytes,1000,opt,name=error,proto3" json:"error,omitempty"`
}
...
func (x *GetCoinSupplyResponseMessage) GetMaxSium() uint64 {
	if x != nil {
		return x.MaxSium
	}
	return 0
...
func (x *GetCoinSupplyResponseMessage) GetCirculatingSium() uint64 {
	if x != nil {
		return x.CirculatingSium
	}
	return 0
}
