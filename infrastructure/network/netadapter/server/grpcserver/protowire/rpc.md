...
Clients are expected to build RequestMessages and wrap them in kestrelcoindMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a kestrelcoindMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a kestrelcoindMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network kestrelcoind is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which kestrelcoin address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether kestrelcoind thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when kestrelcoind is not synced. That is because when kestrelcoind isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known kestrelcoind addresses in the
...
currently connected to this kestrelcoind.
...
| isOutbound | [bool](#bool) |  | Whether this kestrelcoind initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this kestrelcoind |
...
AddPeerRequestMessage adds a peer to kestrelcoind&#39;s outgoing connection list.
This will, in most cases, result in kestrelcoind connecting to said peer.
...
parent chain from some startHash to this kestrelcoind&#39;s current virtual
...
kestrelcoind&#39;s current virtual.
...
GetBlockCountRequestMessage requests the current number of blocks in this kestrelcoind.
...
ShutDownRequestMessage shuts down this kestrelcoind.
...
This call is only available when this kestrelcoind was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given kestrelcoind addresses
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| maxSium | [uint64](#uint64) |  | note: this is a hard coded maxSupply, actual maxSupply is expected to deviate by upto -5%, but cannot be measured exactly. |
| circulatingSium | [uint64](#uint64) |  |  |
| error | [RPCError](#protowire.RPCError) |  |  |
