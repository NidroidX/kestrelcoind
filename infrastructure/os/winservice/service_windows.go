...
// long-running kestrelcoindMain (which is the real meat of kestrelcoind), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start kestrelcoindMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once kestrelcoind is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("kestrelcoindMain-windows", func() {
...
// logServiceStart logs information about kestrelcoind when the main server has
