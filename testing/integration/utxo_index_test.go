...
func TestUTXOIndex(t *testing.T) {
	// Setup a single kestrelcoind instance
	...
	err := kestrelcoind.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := kestrelcoind.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedSium := uint64(blockAmountToMine * constants.SiumPerkestrelcoin * 500)
	getBlockCountResponse, err := kestrelcoind.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountSium := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.SiumPerkestrelcoin * 500, // -2 because of genesis and virtual.
	)

	if getCoinSupplyResponse.CirculatingSium != rewardsMinedSium {
		t.Fatalf("Error: Circulating supply Mismatch - Circulating Sium: %d Sium Mined: %d", getCoinSupplyResponse.CirculatingSium, rewardsMinedSium)
	} else if getCoinSupplyResponse.CirculatingSium != rewardsMinedViaBlockCountSium {
		t.Fatalf("Error: Circulating supply Mismatch - Circulating Sium: %d Sium Mined via Block count: %d", getCoinSupplyResponse.CirculatingSium, rewardsMinedViaBlockCountSium)
	}
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = kestrelcoind.rpcClient.SubmitTransaction(rpcTransaction, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := kestrelcoind.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) *appmessage.RPCTransaction {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixkestrelcoinSim)
	if err != nil {
