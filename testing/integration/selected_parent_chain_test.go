...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of kestrelcoind instances
	kestrelcoind1, kestrelcoind2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := kestrelcoind1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In kestrelcoind1, mine a chain over the genesis and make sure
	// each chain changed notifications contains only one entry
	// in `added` and nothing in `removed`
	chain1TipHash := consensushashing.BlockHash(kestrelcoind1.config.NetParams().GenesisBlock)
	chain1TipHashString := chain1TipHash.String()
	const blockAmountToMine = 10
	for i := 0; i < blockAmountToMine; i++ {
		minedBlock := mineNextBlock(t, kestrelcoind1)
	...
	// In kestrelcoind2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, kestrelcoind2)
	}

	// Connect the two kestrelcoinds. This should trigger sync
	// between the two nodes
	connect(t, kestrelcoind1, kestrelcoind2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := kestrelcoind1.rpcClient.GetVirtualSelectedParentChainFromBlock(
