...
// Outpoint defines a kestrelcoin data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new kestrelcoin transaction outpoint point with the
...
// TxIn defines a kestrelcoin transaction input.
type TxIn struct {
...
// NewTxIn returns a new kestrelcoin transaction input with the provided
...
// TxOut defines a kestrelcoin transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new kestrelcoin transaction output with the provided
...
// MsgTx implements the Message interface and represents a kestrelcoin tx message.
